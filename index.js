// Accessory for controlling ViewSonic Projectors via HomeKit.
...
module.exports = function (homebridge) {
  ...
  homebridge.registerPlatform(
    ...
    "ViewSonic-Projector",
    ViewSonicProjectorPlatform
  );
};

class ViewSonicProjectorPlatform {
  ...
  accessories(callback) {
    ...
    if (!config) {
      log.warn(
        "No config found. Please take a look at the README and example-config.json file of homebridge-viewsonic-projector!"
      );
      callback([]);
      return;
    }
    ...
    if (!config.devices || !(config.devices instanceof Array)) {
      log.warn(
        "Malformed configuration found. Please take a look at the README and example-config.json file of homebridge-viewsonic-projector!"
      );
      callback([]);
      return;
    }

    const tvs = config.devices.map(
      (device) => new ViewSonicProjector(log, device)
    );
    homebridge.publishExternalAccessories(
      "homebridge-viewsonic-projector",
      tvs.map((tv) =>
        createAccessory(tv, tv.name, Categories.TELEVISION, homebridge)
      )
    );
...
class ViewSonicProjector {
  ...
  constructor(log, config) {
    ...
    this.commands = {
      "Power On": "\r*pow=on#\r",
      // "Power On": Buffer.from("0614000400341101005D", "hex"),
      //"Power Off": "\r*pow=off#\r",
      "Power Off": Buffer.from("0614000400341101005E", "hex"),
      "Power State": "\r*pow=?#\r",
      "Mute On": "\r*mute=on#\r",
    ...
    // Other possible inputs but not supported by PX701HD
  ...
  async sendCommands() {
    ...
    for (index in this.queue) {
      ...
      this.log(
        "debug",
        `sendCommand: ${
          Buffer.isBuffer(cmd) ? cmd.toString("hex").toUpperCase() : cmd
        }`
      );
      var response = await serialio
        .send(this.adapter, cmd, {
          baudRate: this.baudrate,
          timeoutInit: 1000,
          timeoutRolling: 1000,
          fixedRespBytesLength: this.state ? 8 : -1,
        })
        .catch((error) => {
          // Don't remove command that failed so it can be run again.
          this.log(
            "debug",
            `Sending command ${cmd} encountered error: ${error}`
          );
        });
      if (response != null) {
        this.log(
          "debug",
          `Response came back: ${response} for command: ${cmd}`
        );
        // Error handling
        ...
        // Response handling
        if (
          cmd === this.commands["Power State"] ||
          response.indexOf("*pow=") > -1
        ) {
  ...
  async handlePowResponse(response) {
    this.log(
      ...
      `handle power response ${
        response === "" ? "empty" : "not-empty"
      } "${response}" (${response.length}) ${this.state ? "ON" : "OFF"}`
    );
    if (response.indexOf("ON") > -1 || response.length === 0) {
      this.log("debug", "Power is On");
      this.state = true;
    }
    if (response.indexOf("OFF") > -1 || response.length !== 0) {
  ...
  setPictureMode(mode, callback) {
    ...
    var cmd =
      this.commands["Picture Mode Set"] + this.pictureModes[mode] + "#\r";
  ...
  getServices() {
    ...
    informationService
      ...
      .setCharacteristic(Characteristic.Manufacturer, "ViewSonic")
